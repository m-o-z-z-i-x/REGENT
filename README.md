...
[![PyPI](https://img.shields.io/pypi/v/regent-mcp?style=for-the-badge)](https://pypi.org/project/regent-mcp/)
...
You never launch the server yourself. The MCP client spawns it, talks to it over stdin and stdout, and shuts it down on exit, so it only has to be registered once. The package lives on PyPI as [`regent-mcp`](https://pypi.org/project/regent-mcp/), and nothing needs installing first — `uvx` fetches it on the first run. After that you describe what you want in ordinary words
...
- **Register with the client** - add this to wherever that client keeps its MCP configuration
  ...
  ```
  Without `uvx`, install it yourself with `pip install regent-mcp` and set `"command": "regent-mcp"` instead
